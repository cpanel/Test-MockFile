...
sub test_content_with_keywords {
    ...
    is( \@content, $dir_content,
        'Correct directory content through Perl core keywords',
    );

    ok( lives( sub { closedir $dh } ), "closedir() on $dirname successful", );
}
...
subtest(
    'Scenario 2: ->dir() on an already existing dir fails made with ->dir()'
        => sub {
        my $dirname = $get_dirname->();
...
subtest(
    'Scneario 3: Undef files with ->file() do not create dirs, adding content changes dir'
        => sub {
        my $dirname = $get_dirname->();
        my $dir     = Test::MockFile->dir($dirname);
        ...
        ok( !-d $dirname,
            "-d $dirname still fails after mocking file with no content" );
        ok( mkdir($dirname), "mkdir $dirname works" );
        ok( -d $dirname,     "-d $dirname now succeeds" );

        is( $dir->contents(), [qw< . .. >], "Correct contents to $dirname", );

        test_content_with_keywords( $dirname, [qw< . .. >] );

        ok( !-e "$dirname/foo",
            "$dirname/foo does not exist, even if $dirname does" );
        $file->contents("hello");
        ok( -e "$dirname/foo",
            "After file->contents(), $dirname/foo exists" );

        is( $dir->contents(), [qw< . .. foo >],
...
subtest(
    'Scenario 4: Creating ->file() with content creates dir' => sub {
        ...
        ok( -d $dirname,
            "$dirname now exists, after creating file with content" );
        ok( !mkdir($dirname),
            "mkdir $dirname fails, since dir already exists" );
