...
subtest(
    'Defaults' => sub {
        ...
        is(
            ...
            $dir_def_perm, "Directory /foo is set to $dir_def_perm",
        );
...
subtest(
    'Changing mode (real vs. mocked)' => sub {
        ...
        ok( CORE::chmod( 0600, 'fooz' ),
            'Successfully chmod\'ed real directory' );
        is(
            sprintf( '%04o', ( CORE::stat('fooz') )[2] & 07777 ),
            '0600', 'CORE::chmod() set the perms correctly',
        );
        ...
        is(
            sprintf( '%04o', ( stat '/foo' )[2] & 07777 ),
            '0600', 'Directory /foo is now set to 0600',
        );

        is(
            sprintf( '%04o', ( stat '/foo/bar' )[2] & 07777 ),
            '0600', 'File /foo/bar is now set to 0600',
        );
        ...
        is(
            sprintf( '%04o', ( stat '/foo' )[2] & 07777 ),
            '0777', 'Directory /foo is now set to 0600',
        );

        is(
            sprintf( '%04o', ( stat '/foo/bar' )[2] & 07777 ),
            '0777', 'File /foo/bar is now set to 0600',
        );
    }
);
...
subtest(
    'Providing a string as mode mask' => sub {
        ...
        like(
            warning(
                sub { $core_chmod_res = CORE::chmod( 'hello', 'fooz' ) }
            ),
        ...
        is(
            sprintf( '%04o', ( CORE::stat('fooz') )[2] & 07777 ),
            '0000', 'CORE::chmod() set the perms correctly',
        );
        ...
        is(
            ...
            '0000', 'chmod() set the perms correctly',
        );
