...
subtest(
    'Defaults' => sub {
        ...
        foreach my $path (qw< /foo /foo/bar >) {
            is(
                ( stat $path )[4],
                $euid, "$path set UID correctly to $euid",
            );

            is(
                ( stat $path )[5],
                $egid, "$path set GID correctly to $egid",
            );
        }
    }
);
...
subtest(
    'Change ownership of file to someone else fails' => sub {
        ok(
    ...
    SKIP: {
            ...
            $> == 0 || grep /(^ | \s ) 0 ( \s | $)/xms, $)
                and skip(
                'Running as root cannot test failing to chown to root' => 9 );

            $! = 0;

            ok( !chown( 0, 0, $filename ), 'Cannot chown file to root', );
            ...
            is(
                ...
                0, 'Cannot chown file to root GID',
            );
            ...
            is(
                ...
                0, 'Cannot chown file to root UID',
            );
...
subtest(
    'chown only user, only group, both' => sub {
        ok( chown( $euid, -1, $filename ), 'chown\'ing file to only UID', );

        ok( chown( -1, $egid, $filename ), 'chown\'ing file to only GID', );
...
subtest(
    'chown with bareword' => sub {
        no strict;
        my $bareword_file
            = Test::MockFile->file('RANDOM_FILE_THAT_WILL_NOT_EXIST');
        ok(
